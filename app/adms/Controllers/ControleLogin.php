    ...
    public function login() {
        ...
        if (!empty($this->Dados['SendLogin'])):
            ...
            if (!$Login->getResultado()):
                ...
                $ActualizarSessao->atualizaSessao($this->Dados[0]['id']);
    ...
    public function recuperarSenha() {
        ...
        if (!empty($this->Dados['SendRecupSenha'])):
            ...
            if ($RecuperarSenha->getResultado()):
                var_dump($RecuperarSenha->getResultado());
                $_SESSION['msg'] = "<div class='alert alert-success'>Enviado dados de recuperação par o seu e-mail!</div>";
                $UrlDestino = URL . 'controle-login/login';
                header("Location: $UrlDestino");
            else:
